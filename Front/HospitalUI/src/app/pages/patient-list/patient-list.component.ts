...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, RouterOutlet, SideBarComponent, HeaderComponent, SearchBarComponent, PaginationComponent, LoaderComponent],
...
export class PatientListComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    searchFilter = this.patients;

    totalPages = Math.ceil(this.patients.length/8);
    rowsPerPage = 8;
    shownOnPage = 0; 
    totalInfo = this.patients.length;
    currentPage = 1; 
